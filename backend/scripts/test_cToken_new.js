...
async function main() {
  ...
  const SimpleCompoundVault = await ethers.getContractFactory("SimpleCompoundVault");
  const simpleCompoundVault = await SimpleCompoundVault.deploy(
    await mintableUSDC.getAddress(),
    "Compound USDC Vault",
    "cUSDC"
  );
  console.log(`SimpleCompoundVault déployé à l'adresse: ${await simpleCompoundVault.getAddress()}`);
  ...
  await mintableUSDC.transferOwnership(await simpleCompoundVault.getAddress());
  console.log(`Droits de mint transférés au vault: ${await simpleCompoundVault.getAddress()}`);
  
  // Définir un taux d'intérêt plus élevé pour le test (30% annuel)
  await simpleCompoundVault.setInterestRate(3000); // 30.00%
  ...
  await mintableUSDC.connect(utilisateur).approve(
    await simpleCompoundVault.getAddress(), 
    montantDepot
  );
  
  // 3. Dépôt dans le vault
  console.log(`Dépôt de ${ethers.formatUnits(montantDepot, 18)} USDC dans le vault...`);
  await simpleCompoundVault.connect(utilisateur).deposit(montantDepot, utilisateur.address);
  
  // Vérification du dépôt
  const sharesUtilisateur = await simpleCompoundVault.balanceOf(utilisateur.address);
  ...
  await simpleCompoundVault.accrueInterest(); // Force l'accumulation d'intérêts
  
  const valeurAvantRetrait = await simpleCompoundVault.convertToAssets(sharesUtilisateur);
  ...
  await simpleCompoundVault.connect(utilisateur).redeem(
    sharesUtilisateur, 
    utilisateur.address, 
    utilisateur.address
  );
