...
//await cToken.approve(await yieldOptimizer.getAddress(), ethers.MaxUint256);
  ...
  await usdc.transfer(deployer.address, ethers.parseUnits("1000", 6));
  ...
  await usdc.connect(deployer).approve(await yieldOptimizer.getAddress(), ethers.parseUnits("1000", 6));
  ...
  // Dépôt initial via YieldOptimizer pour initialiser les vaults
  console.log("Dépôt initial via YieldOptimizer...");
  await yieldOptimizer.connect(deployer).deposit(ethers.parseUnits("1000", 6));
  ...
  // Retrait partiel via YieldOptimizer
