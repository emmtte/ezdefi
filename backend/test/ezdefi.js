...
describe("YieldOptimizer (ezToken) - Tests Combinés", function () {
...
await cToken.setInterestRate(2000); // 20%
  ...
  it("devrait distribuer correctement les bénéfices à plusieurs utilisateurs", async function () {
    await ezToken.connect(utilisateur1).deposit(ethers.parseUnits("1000", 6));
    await ezToken.connect(utilisateur2).deposit(ethers.parseUnits("2000", 6));
  ...
  describe("accrueInterest Edge Cases", function () {
    it("Ne devrait pas accumuler d'intérêts si aucun temps ne s'est écoulé", async function () {
      const depositAmount = ethers.parseUnits("1000", 6);
      await aToken.connect(user1).deposit(depositAmount, user1.address);const initialBalance = await usdc.balanceOf(await aToken.getAddress());
